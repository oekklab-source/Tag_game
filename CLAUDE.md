...
- **新しいテストには必ず `tests/save_guard.gd` の保護ブロック（`_enter_tree()` /
  `_exit_tree()`）を付けること。** 既存の `tests/*.gd` の末尾と同じ形をコピーすればよい。
  ローカルの実セーブの退避・復元に加えて、起動時のクラウドセーブ同期（EOS PDS）も止める。
  ラウンドを回さないテストでも、EOS にログインできる環境では同期が実セーブを書き換え、
  テストがメモリ上で書き換えた値が実アカウントのクラウドへ上がる
  （2026-09-25 に `highest_rating=10004` がクラウドに残っているのを見つけ、手で直した）。
...
表示は日本語と英語。新しい UI 文言は `tr("日本語の原文")` で書き、`locale/en.po` に英訳を足す
（詳細と落とし穴は README の「表示言語」節）。`tests/test_i18n.tscn` が抜けを検出するので、
文言を触ったら必ず流すこと。
