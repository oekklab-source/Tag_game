...
| ギミック | 効果 |
| --- | --- |
| 土管ワープ | ペアの土管の真上へ、進行方向へ軽く飛びながら出る。CPU は近道になる時だけ入る |
...
真上に飛んで**入った時と寸分違わぬ xz 座標へ落ちてくる**。土管の出口は
`EXIT_HEIGHT`(3m) 分だけ相手の口の上に浮いているので、落下の途中で
相手の口（`Mouth` Area、半径2.1m・高さ2.6m）をもう一度通過する。
`warp_lock`(0.9s) は再突入を短時間だけ防ぐが、往復にかかる時間の方が長いため、
**ロックが切れた瞬間にまた入ってしまい A↔B を無限に往復する**。

直すには出口で「進行方向」へ軽い水平の勢いを足せばよい。土管自体には
向きの情報が無い（`WARP_PIPES` にヨー角は無い）ので、**入った瞬間の
...
`tests/warp_pipe.tscn` が「無操作で入っても出口の水平速度がゼロでない」
...
雲とコインは当たらないので同期が要らず、[scenes/decor_float.gdshader](scenes/decor_float.gdshader)
...
tests/warp_pipe.tscn          土管の出口に水平速度が乗るか（無限ループの回帰）
