...
# バナナで足をすくわれて尻もちをつく。長さは banana.gd の STUN(1.5秒) = 45フレームに合わせ、
# 最後のキーを Idle の 0 フレーム目と同じにして、スタンが明けた瞬間の Idle へ滑らかに繋ぐ。
#
# Dive と違って親ノード側は倒さない（rotation.x は Dive 専用で転倒中は 0 のまま）ので、
# 上体を倒すのは Spine/Chest の後傾、腰を落とすのは Root の loc だけで作る。
# 脚は Thigh を大きな負値で前へ投げ出し、腕は正値で後ろへ回して手をつく
SLIP = {
    # 足がすっぽ抜ける。腕が後ろへ跳ね上がり、上体が置いていかれる
    0: {"Root": {"loc": up(-0.005)}, "Spine": (-10, 0, 0), "Chest": (-8, 0, 0),
        "Thigh.L": (-52, 0, 0), "Shin.L": (-6, 0, 0), "Foot.L": (-30, 0, 0),
        "Thigh.R": (-6, 0, 0), "Shin.R": (12, 0, 0), "Foot.R": (-6, 0, 0),
        "UpperArm.L": (40, 0, -50), "UpperArm.R": (40, 0, 50),
        "Hand.L": (0, 0, -10), "Hand.R": (0, 0, 10)},
    # 尻から落ちていく途中。腰が落ちて脚が前に伸び切る
    6: {"Root": {"loc": up(-0.38)}, "Spine": (-26, 0, 0), "Chest": (-16, 0, 0),
        "Thigh.L": (-95, 0, 0), "Shin.L": (-5, 0, 0), "Foot.L": (-30, 0, 0),
        "Thigh.R": (-85, 0, 0), "Shin.R": (5, 0, 0), "Foot.R": (-30, 0, 0),
        "UpperArm.L": (80, 0, -48), "UpperArm.R": (80, 0, 48),
        "Hand.L": (0, 0, -16), "Hand.R": (0, 0, 16)},
    # 着尻。潰れて脚が跳ね上がり、頭だけ勢いで前へ振られる
    12: {"Root": {"loc": up(-0.47)},
         "Spine": {"rot": (-30, 0, 0), "scale": (1.05, 0.94, 1.05)}, "Chest": (-6, 0, 0),
         "Thigh.L": (-105, 0, 0), "Shin.L": (20, 0, 0), "Foot.L": (-34, 0, 0),
         "Thigh.R": (-98, 0, 0), "Shin.R": (26, 0, 0), "Foot.R": (-34, 0, 0),
         "UpperArm.L": (72, 0, -38), "UpperArm.R": (72, 0, 38),
         "Hand.L": (0, 0, -22), "Hand.R": (0, 0, 22)},
    # 反動。潰れが戻って上体が起きる
    16: {"Root": {"loc": up(-0.375)},
         "Spine": {"rot": (-18, 0, 0), "scale": (0.97, 1.04, 0.97)}, "Chest": (-14, 0, 0),
         "Thigh.L": (-92, 0, 0), "Shin.L": (34, 0, 0), "Foot.L": (-20, 0, 0),
         "Thigh.R": (-88, 0, 0), "Shin.R": (30, 0, 0), "Foot.R": (-20, 0, 0),
         "UpperArm.L": (66, 0, -34), "UpperArm.R": (66, 0, 34),
         "Hand.L": (0, 0, -18), "Hand.R": (0, 0, 18)},
    # 座り込み。後ろ手で体を支えたまま止まる（ここが一番長い）
    26: {"Root": {"loc": up(-0.31)}, "Spine": (-22, 0, 0), "Chest": (-8, 0, 0),
         "Thigh.L": (-88, 0, 0), "Shin.L": (40, 0, 0), "Foot.L": (-14, 0, 0),
         "Thigh.R": (-84, 0, 0), "Shin.R": (46, 0, 0), "Foot.R": (-14, 0, 0),
         "UpperArm.L": (70, 0, -30), "UpperArm.R": (70, 0, 30),
         "Hand.L": (0, 0, -18), "Hand.R": (0, 0, 18)},
    # 膝を立てて前かがみになり、立ち上がりにかかる
    36: {"Root": {"loc": up(-0.10)}, "Spine": (2, 0, 0), "Chest": (4, 0, 0),
         "Thigh.L": (-62, 0, 0), "Shin.L": (78, 0, 0), "Foot.L": (10, 0, 0),
         "Thigh.R": (-58, 0, 0), "Shin.R": (74, 0, 0), "Foot.R": (10, 0, 0),
         "UpperArm.L": (30, 0, -20), "UpperArm.R": (30, 0, 20)},
    # 立ち上がり途中
    42: {"Root": {"loc": up(-0.005)}, "Spine": (6, 0, 0), "Chest": (2, 0, 0),
         "Thigh.L": (-26, 0, 0), "Shin.L": (38, 0, 0), "Foot.L": (6, 0, 0),
         "Thigh.R": (-24, 0, 0), "Shin.R": (36, 0, 0), "Foot.R": (6, 0, 0),
...
# 「カモン！」= 右腕を横に上げて手首で招く。
# 体をわずかに右へひねって「呼んでいる向き」を出す。左腕は下ろしたまま静かにする
COME_OUT = {
    "UpperArm.R": (-26, 0, 98), "Hand.R": (-40, 0, 0),
    "UpperArm.L": (6, 0, -10), "Hand.L": (-16, 0, 0),
    "Spine": {"rot": (-2, 4, 0)}, "Chest": {"rot": (-5, 8, 0)},
    "Thigh.L": (-4, 0, 0), "Thigh.R": (-4, 0, 0),
    "Root": {"loc": up(0.008)},
}
COME_IN = {
    "UpperArm.R": (-44, 0, 80), "Hand.R": (38, 0, 0),
    "UpperArm.L": (6, 0, -10), "Hand.L": (-16, 0, 0),
    "Spine": {"rot": (-5, 2, 0), "scale": (1.018, 0.972, 1.018)},
    "Chest": {"rot": (-10, 5, 0)},
    "Thigh.L": (-8, 0, 0), "Shin.L": (12, 0, 0),
    "Thigh.R": (-8, 0, 0), "Shin.R": (12, 0, 0),
    "Root": {"loc": up(-0.016)},
}
...
CLIPS = {
    ...
    # 0.67秒で2回招く。ナイスより速いのは「急いで呼んでいる」ことを動きで伝えるため
    "Come": ({0: COME_OUT, 5: COME_IN, 10: COME_OUT, 15: COME_IN, 20: COME_OUT}, True),
}
