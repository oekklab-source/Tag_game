...
func refresh() -> void:
	...
	if not any_online:
		online_list.add_child(_build_empty_label(tr("オンラインのフレンドはいません")))
	if not any_offline:
		offline_list.add_child(_build_empty_label(tr("オフラインのフレンドはいません")))

	if not EosManager.is_eos_available:
		status_label.text = tr("EOSに接続されていないため、フレンド一覧はサンプル表示です。")
	else:
		status_label.text = ""
...
func _build_search_section() -> void:
	...
	_search_mode.add_item(tr("名前で検索"))
	_search_mode.add_item(tr("コードで検索"))
...
func _on_search_pressed() -> void:
	...
	if not res.get("found", false):
		...
		var msg := tr("検索回数の上限に達しました。しばらくしてから試してください。") \
			if reason == "rate_limited" else tr("見つかりませんでした。")
...
func _build_search_result_row(m: Dictionary) -> Control:
	...
	name_lbl.text = tr("%s（コード: %s）") % [String(m.get("name", "")), String(m.get("code", ""))]
...
func _on_add_from_search_pressed(code: String, source_btn: Button) -> void:
	...
	if res.get("ok", false):
		status_label.text = tr("%s さんにリクエストを送りました。") % String(res.get("target_name", tr("フレンド")))
		source_btn.disabled = true
		source_btn.text = tr("送信済み")
	else:
		status_label.text = tr("リクエストの送信に失敗しました。")


func _sync_my_code() -> void:
	var code := await FriendManager.sync_with_backend()
	_my_code_label.text = code if not code.is_empty() else tr("取得失敗")
...
func _build_remove_confirm_dialog() -> void:
	...
	_remove_confirm_dialog.title = tr("フレンド削除の確認")
	_remove_confirm_dialog.ok_button_text = tr("削除する")
	_remove_confirm_dialog.cancel_button_text = tr("キャンセル")
...
func _build_friend_detail_dialog() -> void:
	...
	_detail_dialog.title = tr("フレンド情報")
	_detail_dialog.ok_button_text = tr("閉じる")
...
func _on_show_detail_pressed(friend_puid: String) -> void:
	...
	if not profile.get("ok", false):
		status_label.text = tr("フレンド情報の取得に失敗しました。")
		return
	_detail_name_lbl.text = String(profile.get("name", "Friend"))
	_detail_name_lbl.auto_translate_mode = Node.AUTO_TRANSLATE_MODE_DISABLED  # プレイヤー名は利用者の入力(L-09)
	var online := bool(profile.get("online", false))
	_detail_online_lbl.text = tr("オンライン") if online else tr("オフライン")
	...
	if profile.get("stats_available", false):
		...
		_detail_rating_lbl.text = tr("レート: [%s] %d Pt") % [tr(RankingManager.tier_name(rating)), rating]
		var matches := int(profile.get("matches_played", 0))
		_detail_record_lbl.text = tr("戦績: %d戦（にげる勝ち %d / おに勝ち %d）") % \
		...
		_detail_rating_lbl.text = tr("戦績データがまだありません。")
...
func _format_last_seen(last_seen_ms: int, online: bool) -> String:
	...
	if elapsed_sec < 60:
		return tr("最終ログイン: たった今")
	if elapsed_sec < 3600:
		return tr("最終ログイン: %d分前") % (elapsed_sec / 60)
	if elapsed_sec < 86400:
		return tr("最終ログイン: %d時間前") % (elapsed_sec / 3600)
	return tr("最終ログイン: %d日前") % (elapsed_sec / 86400)
...
func _refresh_requests() -> void:
	...
	if requests.is_empty():
		_requests_list.add_child(_build_empty_label(tr("届いているリクエストはありません")))
		return
...
func _on_invite_pressed(friend_name: String) -> void:
	if await FriendManager.invite_to_lobby():
		status_label.text = tr("接続先をコピーしました。%s さんにDirectConnectタブへ貼り付けてもらってください。") % friend_name
	else:
		status_label.text = tr("招待に失敗しました(ロビーに参加していないか、接続先アドレスがまだ準備できていません)。")
...
func _confirm_remove_friend(friend_puid: String, friend_name: String) -> void:
	...
	_remove_confirm_dialog.dialog_text = tr("%s さんをフレンドから削除しますか？") % friend_name
...
func _on_remove_confirmed() -> void:
	...
	if await FriendManager.remove_friend(friend_puid):
		status_label.text = tr("%s さんをフレンドから削除しました。") % friend_name
		await refresh()
	else:
		status_label.text = tr("%s さんの削除に失敗しました。") % friend_name
...
func _on_add_friend_pressed() -> void:
	...
	if res.get("ok", false):
		...
		status_label.text = tr("リクエストの送信に失敗しました(コードが正しいか確認してください)。")
...
func _on_copy_code_pressed() -> void:
	...
	status_label.text = tr("フレンドコードをコピーしました。")
